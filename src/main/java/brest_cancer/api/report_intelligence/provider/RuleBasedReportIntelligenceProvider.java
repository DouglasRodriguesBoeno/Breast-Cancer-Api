...
@Component
public class RuleBasedReportIntelligenceProvider implements ReportIntelligenceProvider {
    ...
    public ReportIntelligenceResult analyze(AnalyzeReportRequest request) {
        ...
        return new ReportIntelligenceResult(
                ...
                structuredFindings,
                importantTerms,
                buildEducationalSummary(structuredFindings, reportType),
                buildSimpleExplanation(structuredFindings),
                wdbcCompatibility,
                buildSafetyNotes(),
                "RULE_BASED",
                "report-intelligence-rule-based-v1"
        );
    }
    ...
    private StructuredFindings extractStructuredFindings(String reportText, String normalizedText) {
        return new StructuredFindings(
                extractBirads(reportText),
                extractBreastSide(normalizedText),
                extractLocation(normalizedText),
                extractMeasurements(reportText),
                extractMentionedFindings(normalizedText),
                extractMentionedRecommendations(normalizedText)
        );
    }
